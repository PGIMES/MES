...
public partial class Forms_PurChase_PO_Deal_Form : System.Web.UI.Page
{
    ...
    public static string deal(string ids, string ponos)
    {
        ...
        string sql_po_price = @"select a.PONo,a.TotalPrice-b.TotalPrice TotalPrice
                            from (select pono,sum(TotalPrice) TotalPrice from PUR_PO_Dtl_Form where pono in({0}) and status=0 group by pono) a
	                            inner join(select pono,sum(TotalPrice) TotalPrice from PUR_PO_Dtl_Form where id in({1}) group by pono) b on a.PONo=b.PONo";
        sql_po_price = string.Format(sql_po_price, ponos, ids);
        ...
        sql_update_po_main = string.Format(sql_update_po_main, sql_po_price);
        ...
        sql_update_ht_his = string.Format(sql_update_ht_his, sql_po_price);
        ...
        sql_update_ht = string.Format(sql_update_ht, sql_po_price);
