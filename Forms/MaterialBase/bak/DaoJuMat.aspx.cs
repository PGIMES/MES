...
public partial class DaoJuMat : PGIBasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (wlh != ""&& Request["wlh"]!=null && domain != "" && Request["domain"]!=null)
        {
            ...
            dtMstOld = DbHelperSQL.Query("select *,'是' as askprice,aqkc as oldaqkc from PGI_BASE_PART_DATA where wlh='" + wlh.ToString() + "' and domain='" + domain.ToString() + "'").Tables[0];
        ...
        if (dtMstOld != null && dtMstOld.Rows.Count > 0)
        {
            ...
            oldaqkc.Text = dtMstOld.Rows[0]["oldaqkc"].ToString();
        ...
        //strBSql.Append(" +'){ $(\"#edsm\").val(\"'+cast(edsm as varchar)+'\") }'   from PGI_BASE_PART_DATA_lifetime where 1=1 and isYX='Y'");
        ...
        sqlScript ="select control_event as script from auto_form where form_type='" + type.SelectedValue + "' and isnull(control_id,'')<>'' and isnull(control_event,'')<>'' order by form_div, control_order;";
