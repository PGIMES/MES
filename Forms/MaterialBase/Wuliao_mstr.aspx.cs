...
public partial class Forms_MaterialBase_Wuliao_mstr : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
            ...
            if (LogUserModel != null)
            {
                ...
                lssql = @"select null id, formno,buyer_planner,isfx,a.site,fxcode,dhff,ddsl,dhperiod,pt_pm_code,make_days,domain,
                    ...
                    if (state == "edit" )//编辑
                    {
                        ...
                        lssql = @"select null id, formno,buyer_planner,isfx,a.site,fxcode,dhff,ddsl,dhperiod,pt_pm_code,make_days,domain,
                                 purchase_days,quantity_min,quantity_max,ddbs,aqtj_wuliu ,ROW_NUMBER() OVER(ORDER BY id) numid from PGI_PartDtl_DATA a";
        ...
        if (dtMst != null && dtMst.Rows.Count > 0)
        {
           ...
           // ViewState["line"] = line.Substring(0, 1);
            ...
            var tbltype = DbHelperSQL.Query("  select distinct   pl_prod_line as value,  pl_desc+'('+pl_prod_line+')' as text from qad.dbo.qad_pl_mstr where pl_prod_line='" + line + "'").Tables[0];
    ...
    {
        string sql = "select top 1 * from PGI_PartMstr_DATA_Form where formno='" + Request["formid"] + "'";
        ...
        else if ((string)ViewState["UidRole"] == "jihua" && (string)ViewState["line"] != "1")
        {
  ...
  [System.Web.Services.WebMethod()]
    public static string CheckData(string xmh, string cpleibie, string gc, string bz, string comp, string formno, string hz, string state, string status, string aprover)
    {
        string check_flag = "";
        DataTable dt = DbHelperSQL.Query(@"select LEFT(bz_user,5)bz_user,left(caigou,5)caigou,left(product_user,5) product_user from form3_Sale_Product_MainTable where pgino='" + xmh + "' ").Tables[0];
        if (dt.Rows[0][1].ToString() == "" && cpleibie == "1")
        {
            check_flag = "产品清单中(" + xmh + ")采购工程师不存在，不能提交!<br />";
            
        }
        if ((dt.Rows[0][0].ToString() == ""  || dt.Rows[0][2].ToString()=="")&& cpleibie != "1")
        {
            check_flag = "产品清单中(" + xmh + ")包装工程师或产品工程师不存在，不能提交!<br />";
        }
        string wlh = hz == "" ? xmh + gc + bz : xmh + gc + bz + "-" + hz;
        ...
        if (dt1.Rows.Count > 0 && state != "edit" && status == "新增" && aprover == "")
        {
            check_flag = "物料号: " + wlh+" 已存在,请至查询页面编辑后送签！<br />";
        }
        if (re_dt.Rows.Count > 0 && formno == "")
        {
            check_flag = "物料号: " + wlh + ";正在申请中,单号:" + re_dt.Rows[0]["InstanceID"] + "不能提交<br />";
        }
  ...
  public static string Getbuyer(string P1, string P2)
  {
      ...
      sql = string.Format("     select ''value,''text union all select value as value,val as text from pt_base_code where type='pt_buyer' and substring(val,2,1)='0' and text='"+P2+"'   and domain like '" + P1 + "%'  ");
    ...
    public static string GetDL(string P1,string P2)
    {
        ...
        sql = string.Format("  select distinct   pl_desc+'('+pl_prod_line+')'  as text,pl_prod_line as value from qad.dbo.qad_pl_mstr where pl_desc like '%{0}%' and pl_domain='{1}'  ", P1, P2);
    ...
    public static void CheckData_uid(string xmh, string cpl, out string flag)
    {
        ...
        {
            flag = "产品清单中(" + xmh + ")采购工程师不存在，不能提交!<br />";
        }
        if ((dt.Rows[0][1].ToString() == "" || dt.Rows[0][2].ToString() == "") && cpl != "1")
        {
            flag = "产品清单中(" + xmh + ")包装工程师或产品工程师不存在，不能提交!<br />";
        }
    ...
    {   //保存数据是否成功标识
       ...
       // CheckData_uid( xmh, cpl,out  check_flag);
        ...
        DataTable dt = DbHelperSQL.Query(@"select left(caigou,5)caigou,left(bz_user,5)bz_user,left(product_user,5) product_user from form3_Sale_Product_MainTable where pgino='" + wlh + "' ").Tables[0];
        ...
        if (instanceid > 0)
        {
            var titletype = state == "edit" ? "物料修改" : "物料申请";
            string title = titletype + "[" + m_sid+ "][" + Request.Form["wlh"]+"][" + Request.Form["wlmc"] + "]"; //设定表单标题
    ...
    protected void GetGrid(DataTable DT)
    {
      ...
      //  string comp =DT.Rows.Count>0? DT.Rows[0]["domain"].ToString():"";
        ...
        for (int i = 0; i < gv_d.VisibleRowCount; i++)
        {
            ...
            DataTable ldt_ps = DbHelperSQL.Query(@"select ''cateid,''catevalue union all select value as cateid,val as catevalue from pt_base_code where type='pt_buyer'  and domain like '"+comp+"%'   ").Tables[0];
    ...
    private void GvAddRows(int lnadd_rows, int lnindex)
    {
        ...
        for (int i = 0; i < lnadd_rows; i++)
        {
            ...
            for (int j = 0; j < ldt.Columns.Count; j++)
            {
                ...
                if (ldt.Columns[j].ColumnName == "pt_pm_code")
                {
