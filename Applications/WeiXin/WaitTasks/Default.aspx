<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Default.aspx.cs" Inherits="WebForm.Applications.WeiXin.WaitTasks.Default" %>
    ...
    <div class="weui-cells" style="overflow-y:auto; overflow-x:hidden;">
        ...
        foreach(var task in TasksList)
        {
            var address = RoadFlow.Utility.Config.BaseUrl + "/Platform/WorkFlowRun/Default_App.aspx?flowid=" + task.FlowID
                + "&stepid=" + task.StepID + "&instanceid=" + task.InstanceID + "&taskid=" + task.ID + "&groupid=" + task.GroupID + "&ismobile=1";
        %>
        <a class="weui-cell weui-cell_access" href="<%=address %>">
            <div class="weui-cell__bd">
                <p><%=task.Title %><p class="date">接收时间:<%=task.ReceiveTime.ToDateTimeString() %> 发送人:<%=task.SenderName %></p></p>
            </div>
                ...
                $.ajax({
                    ...
                    cache: false, dataType: "json", success: function (json)
                    {
                        ...
                        for (var i = 0; i < json.length; i++)
                        {
                            ...
                            + "&stepid=" + json[i]["stepid"] + "&instanceid=" + json[i]["instanceid"] + "&taskid=" + json[i]["id"] + "&groupid=" + json[i]["groupid"] + "&ismobile=1";
                            html += '<a class="weui-cell weui-cell_access" href="' + address + '">';
                            html += '<div class="weui-cell__bd">';
                            html += '<p>' + json[i]["title"] + '<p class="date">接收时间:' + json[i]["time"] + ' 发送人:' + json[i].sender + '</span></p>';
                            html += '</div>';
