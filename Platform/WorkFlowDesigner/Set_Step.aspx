...
<body style="overflow:hidden;">
    ...
    <table cellpadding="0" cellspacing="1" border="0" width="99%" class="formtable">
        ...
        <tr>
            ...
            <th>数据保存方式：</th>
            <td>
                <select class="myselect" onchange="if(this.value=='0'){$('#base_sqlwheretr').hide();}else{$('#base_sqlwheretr').show();}" style="width:75%;" id="base_DataSaveType">
                    <option value="0">共同编辑</option>
                    <option value="1">独立编辑</option>
                </select>
            </td>
        </tr>
        <tr>
        ...
        <tr id="base_sqlwheretr" style="display:none;">
            <th>SQL条件：</th>
            <td colspan="3"><textarea id="base_DataSaveType_Where" class="mytext" style="width:89%; height:30px;"></textarea></td>
        </tr>
        <tr>
            <th>表单：</th>
            <td colspan="3">
        ...
        <tr>
            <th>说明：</th>
            <td colspan="3"><textarea id="base_Note" class="mytext" style="width:89%; height:40px;"></textarea></td>
        </tr>
    </table>
    </div>
    ...
    <table cellpadding="0" cellspacing="1" border="0" width="99%" class="formtable">
        ...
        <tr>
            <th>会签策略：</th>
            <td>
                <select class="myselect" style="width:210px;" id="behavior_Countersignature">
    ...
    <div style="height:8px;"></div>
    <div id="button_List" style="width:220px; height:270px; overflow-y:auto; overflow-x:hidden; border:1px solid #e8e8e8; padding:3px; float:left;">
    <div style="width:94%;">  
    ...
    {
    %>
        <ul class="listulli" style="width:191px;" note="<%=button.Note %>" title="<%=button.Note %>" val="<%=button.ID %>" onmouseover="$(this).removeClass().addClass('listulli1');" onmouseout="if($currentButton==null || $currentButton.get(0)!==this){$(this).removeClass().addClass('listulli');}" onclick="button_click(this);" ondblclick="button_dblclick(this)" >
            <%if (button.Ico.IsFontIco()){  %>
            <i style="font-size:14px; margin-right:3px;" class="fa <%=button.Ico%>"></i><label style=""><%=button.Title %></label>
            <%}
            else
            { %>
            <label style="background:url(../../<%=button.Ico%>) no-repeat left;padding-left:20px;"><%=button.Title %></label>
            <%}%>
        </ul>
    <%}%>
    </div>
    ...
    <div id="button_Select" style="width:220px; height:270px; overflow-y:auto; overflow-x:hidden; border:1px solid #e8e8e8; padding:3px; float:left;">
        <div style="width:94%;"></div>
    </div>
    <div style="width:20px; height:270px; float:left;"></div>
    <div id="button_Note" style="width:150px; height:270px; overflow-y:auto; overflow-x:hidden; border:1px solid #e8e8e8; padding:3px; float:left; overflow:auto;">
    ...
    <div style="width:100%; height:256px; overflow-y:auto; overflow-x:hidden;">
    ...
    <script type="text/javascript">
        ...
        {
            $.ajax({ url: "../Menu/GetApps.ashx", data: { type: value }, async: false, type: "post", success: function (txt)
            {
        ...
        function initStep(step)
        {
            ...
            if(step.dataSaveType) $("#base_DataSaveType").val(step.dataSaveType);
            if("0"==step.dataSaveType)
            {
                $("#base_sqlwheretr").hide();
            }
            else if("1"==step.dataSaveType)
            {
                $("#base_sqlwheretr").show();
            }
            if(step.dataSaveTypeWhere) $("#base_DataSaveType_Where").val(step.dataSaveTypeWhere);
            if(step.dataSaveTypeWhere) $("#base_DataSaveType_Where").val(step.dataSaveTypeWhere);
        ...
        function confirm1()
        {
            ...
            step.dataSaveType = $("#base_DataSaveType").val() || "";
            step.dataSaveTypeWhere = $("#base_DataSaveType_Where").val() || "";
            ...
            {
                step.forms.push({ id: form_forms, name:"", type:form_type, idApp:form_formsApp, nameApp:"", typeApp:form_type1,  srot:0 });
            }
