...
function getTables(connid)
{
    var tables = [];
    $.ajax({
        url: RoadUI.Core.rooturl() + "/Platform/WorkFlowDesigner/GetTables.ashx?connid=" + connid, dataType: "json", async: false, cache: false, success: function (json)
        {
...
function getFields(connid, table)
{
    var fields = [];
    $.ajax({
        url: RoadUI.Core.rooturl() + "/Platform/WorkFlowDesigner/GetFields.ashx?connid=" + connid + "&table=" + table, dataType: "json", async: false, cache: false, success: function (json)
        {
            for (var i = 0; i < json.length; i++)
            {
                fields.push(json[i]);
...
function addStep1(step)
{
    ...
    for (var i = 0; i < wf_json.steps.length; i++)
    {
        ...
        {
            removeArray(wf_json.steps, i);
            //wf_json.steps.remove(i);
        }
    }
    if (!isup)
    {
...
{
    var bbox = this.getBBox();
    var url = "/Platform/WorkFlowDesigner/Set_Step.aspx?appid=" + appid + "&id=" + this.id + "&x=" + bbox.x + "&y=" + bbox.y + "&width=" + bbox.width + "&height=" + bbox.height;
    dialog.open({ title: "步骤参数设置", width: 700, height: 430, url: url, opener:window, openerid: iframeid, resize: false });
}
...
{
    var bbox = this.getBBox();
    var url = "/Platform/WorkFlowDesigner/Set_SubFlow.aspx?appid=" + appid + "&id=" + this.id + "&x=" + bbox.x + "&y=" + bbox.y + "&width=" + bbox.width + "&height=" + bbox.height;
    dialog.open({ title: "子流程步骤参数设置", width: 700, height: 400, url: url, opener: window, openerid: iframeid, resize: false });
}

//流转条件设置
function connSetting()
{
    var url = "/Platform/WorkFlowDesigner/Set_Line.aspx?appid=" + appid + "&id=" + this.id + "&from=" + this.fromid + "&to=" + this.toid;
    dialog.open({ title: "流转条件设置", width: 700, height: 400, url: url, opener: window, openerid: iframeid, resize: false });
}

//流程属性设置
function flowAttrSetting(isAdd, typeid)
{
    var url = "/Platform/WorkFlowDesigner/Set_Flow.aspx?appid=" + appid + "&isadd=" + (isAdd || 0).toString() + "&flowid=" + (wf_json ? wf_json.id : "") + "&typeid=" + (typeid || "");
    dialog.open({ title: "流程属性设置", width: 550, height: 390, url: url, opener: window, openerid: iframeid, resize: false });
}

//打开流程
function openFlow()
{
    var url = "/Platform/WorkFlowDesigner/Open.aspx?appid=" + appid;
    dialog.open({ title: "打开流程", width: 850, height: 450, url: url, opener: window, openerid: iframeid, resize: false });
}

function openFlow1(id)
{
    var json = $.ajax({
        url: RoadUI.Core.rooturl() + "/Platform/WorkFlowDesigner/GetJSON.ashx?appid=" + appid + "&flowid=" + id, async: false, cache: false, dataType: "json", success: function (json)
        {
            reloadFlow(json);
        }
    });
...
//新建流程
function addFlow(typeid)
{
    flowAttrSetting(1, typeid);
}
...
function saveFlow(op)
{
    ...
    var url = "/Platform/WorkFlowDesigner/Opation.aspx?appid=" + appid + "&flowid=" + wf_json.id + "&op=" + (op || "save");
...
function saveAs()
{
    ...
    var url = "/Platform/WorkFlowDesigner/SaveAs.aspx?appid=" + appid + "&flowid=" + wf_json.id;
    dialog.open({ title: "流程另存为", width: 600, height: 230, url: url, opener: window, openerid: iframeid, resize: false });
}
