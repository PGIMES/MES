...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    <title></title>
    <script type="text/javascript" src="Scripts/common.js?ver=2" ></script>
</head>
<body>
<%
    ...
    if ("0" == display)
    {
        if (task != null)
        {
            ...
            else if ("1" != display && task.ReceiveID != currentUserID)
            {
...
<form id="mainform" name="mainform" method="post" target="submiter">
    ...
    <div class="toolbar" style="margin-top:0; border-top:none 0; position:fixed; top:0; left:0; right:0; margin-left:auto; z-index:999; width:100%; margin-right:auto; height:30px;text-align:right;display:none"><!--此处有被隐藏 by fish:2018.01.30-->
        <div>
        <%
        //如果是抄送，只显示完成按钮
        ...
        foreach (var button in buttons)
        {
         ...
         %>
            <a href="javascript:void(0);" id='<%=button1.ID.ToString().ToUpper() %>'  onclick="<%=funName %>;return false;" title="<%=!button.Note.IsNullOrEmpty() ? button.Note : button1.Note.IsNullOrEmpty() ? "" : button1.Note.Replace("\"", "'") %>">
                ...
                <%}else{ %>
                <span style="background:url(<%=WebForm.Common.Tools.BaseUrl + button1.Ico%>) no-repeat left center;"><%=showTitle %></span>
                <%} %>
            </a>
    ...
    <div style="height:40px;display:none"></div> <!--此处有被隐藏 by fish:2018.01.30-->
    ...
    <!--设定iframe高度自适应 added by fish 2017.1.31--> 
     <script>
        function iframeLoad()  
        {  
            document.getElementById("customeformiframe").height=0;  
            document.getElementById("customeformiframe").height=document.getElementById("customeformiframe").contentWindow.document.body.scrollHeight;  
        } 
    </script>
    <!--表单主体--> 
    ...
    %>
    <div class="toolbar" style="margin-top:0; border:none 0; background:#fff; position:fixed; top:35px; left:0; right:0; margin-left:auto; z-index:999; width:98%; margin-right:auto; height:30px;display:none"><!--此处有被隐藏 by fish:2018.01.30-->
        <div style="margin:8px 0 0 0;">
        处理意见：<select class="myselect" id="mycomment" style="margin-right:6px; width:100px;" onchange="$('#comment').val(this.value);"><option value=""></option><%=commentsOptions %></select>
        &nbsp;<input type="text" class="mytext" id="comment" name="comment" value="" style="width:60%; margin-right:6px;" />
        &nbsp;<input type="text" class="myfile" value="" id="flowfiles" name="flowfiles" />
        <%if (signType == 2){%>
...
<script type="text/javascript">
    ...
    var isSign = '<%=isSign%>' == 'True' && "True"!="<%=isCopyFor%>";
