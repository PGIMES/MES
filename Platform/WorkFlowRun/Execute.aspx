...
<% 
    ...
    // Response.Write("<script type=\"text/javascript\">alert('" + msg + "');if(top.mainDialog){top.mainDialog.close();}</script>");
    Response.Write("<script type=\"text/javascript\">;if(top.mainDialog){top.mainDialog.close();}</script>");
    if (reslut.IsSuccess)
    {
        //判断是打开任务还是关闭窗口
        ...
        if (nextTask != null)
        {
            string url = string.Format("{0}?flowid={1}&stepid={2}&taskid={3}&groupid={4}&instanceid={5}&appid={6}&tabid={7}&ismobile={8}",
                ismobile ? "Default_App.aspx" : "Default.aspx", nextTask.FlowID, nextTask.StepID, nextTask.ID, nextTask.GroupID, nextTask.InstanceID,
                Request.QueryString["appid"], Request.QueryString["tabid"], ismobile ? "1" : ""
                );
            Response.Write("<script type=\"text/javascript\">if(top.refreshPage){top.refreshPage();}window.parent.location = '" + url + "';</script>");
        }
        else
        {
            if (ismobile)
            {
                Response.Write("<script type=\"text/javascript\">top.location='../../Applications/WeiXin/WaitTasks/Default.aspx';</script>");
            }
            else
            {
                Response.Write("<script type=\"text/javascript\">if(top.refreshPage){top.refreshPage();}if(top.mainTab){top.mainTab.closeTab();}else{ top.close();}</script>");
            }
        }
    }
%>
