...
function flowSend(isSubmit)
{
    ...
    {         
        ...
        var url = "/Platform/WorkFlowRun/FlowSend.aspx?" + query + "&instanceid1=" + $("#instanceid").val();
        //var url = "./FlowSend.aspx?" + query + "&instanceid1=" + $("#instanceid").val();
        mainDialog.open({ url: url, openerid: iframeid, width: ismobile ? 300 : 500, height: 300, title: "发送" });
    }
}
...
function flowFreedomSend(isSubmit)
{
    ...
    {
        var mainDialog = top && top.mainDialog ? top.mainDialog : new RoadUI.Window();
        var url = "/Platform/WorkFlowRun/FlowFreedomSend.aspx?" + query + "&instanceid1=" + $("#instanceid").val();
        mainDialog.open({ url: url, openerid: iframeid, width: ismobile ? 300 : 500, height: 300, title: "自由发送" });
    }
}
...
function flowBack(isSubmit)
{
    ...
    var mainDialog = top && top.mainDialog ? top.mainDialog : new RoadUI.Window();
    var url = "/Platform/WorkFlowRun/FlowBack.aspx?" + query;
    mainDialog.open({ url: url, openerid: iframeid, width: ismobile ? 300 : 500, height: 300, title: "退回" });
}
...
{
    var mainDialog = top && top.mainDialog ? top.mainDialog : new RoadUI.Window();
    mainDialog.open({ url: "/Platform/WorkFlowRun/FlowRedirect.aspx?" + query, openerid: iframeid, width: ismobile ? 300 : 500, height: 200, title: "选择接收人员" });
}

function flowCopyFor()
{
    var mainDialog = top && top.mainDialog ? top.mainDialog : new RoadUI.Window();
    mainDialog.open({ url: "/Platform/WorkFlowRun/FlowCopyFor.aspx?" + query, openerid: iframeid, width: ismobile ? 300 : 500, height: 200, title: "选择抄送人员" });
}
...
function formSubmit(options)
{
    ...
    //{
        ...
        f.action = RoadUI.Core.rooturl() + "/Platform/WorkFlowRun/Execute.aspx?" + query + "&isSystemDetermine=" + isSystemDetermine;
        f.submit();
    //}
}
...
function saveData(opation)
{
    ...
    var f = document.forms[0];
    window.setTimeout('', 100);
    f.action = RoadUI.Core.rooturl() + "/Platform/WorkFlowRun/SaveData.aspx?" + query + "&opation=" + opation;
    f.submit();
}
...
{
    var mainDialog = top && top.mainDialog ? top.mainDialog : new RoadUI.Window();
    mainDialog.open({ title: "请输入签章密码", width: ismobile ? 300 : 360, height: 130, url: "/Platform/WorkFlowRun/Sign.aspx?appid=" + appid + "&buttonid=" + (id || ""), openerid: iframeid, resize: false });
}
...
{
    var mainDialog = top && top.mainDialog ? top.mainDialog : new RoadUI.Window();
    mainDialog.open({ id: 'showprocess', title: '查看处理过程', url: '/Platform/WorkFlowTasks/Detail.aspx?' + query, width: ismobile ? 320 : 1024, height: 550 });
}

function showFlowDesign()
{
    var mainDialog = top && top.mainDialog ? top.mainDialog : new RoadUI.Window();
    mainDialog.open({ id: 'showflowdesign', title: '查看流程图', url: '/Platform/WorkFlowRun/ShowDesign.aspx?' + query, width: ismobile ? 320 : 1024, height: 550 });
}

function formPrint()
{
    RoadUI.Core.open(RoadUI.Core.rooturl() + "/Platform/WorkFlowRun/Print.aspx?" + query + "&instanceid1=" + $("#instanceid").val() + "&isreadonly=1&display=1", ismobile ? 320 : 980, 600, "打印表单");
}

function showSubFlow()
{
    var mainDialog = top && top.mainDialog ? top.mainDialog : new RoadUI.Window();
    mainDialog.open({ id: 'showsubflow', title: '查看子流程处理过程', url: '/Platform/WorkFlowTasks/DetailSubFlow.aspx?' + query, width: ismobile ? 320 : 1024, height: 550 });
}
...
{
    var mainDialog = top && top.mainDialog ? top.mainDialog : new RoadUI.Window();
    mainDialog.open({ id: 'showmainflowform', title: '查看主流程表单', url: '/Platform/WorkFlowRun/ShowForm.aspx?' + query, width: ismobile ? 320 : 1024, height: 550 });
}
//加签
function addWrite()
{
    var mainDialog = top && top.mainDialog ? top.mainDialog : new RoadUI.Window();
    mainDialog.open({ id: 'addWrite', title: '加签', url: '/Platform/WorkFlowRun/AddWrite.aspx?' + query, width: ismobile ? 320 : 500, height: 350 });
}