formrun = {
    ...
    initData: function (jsonData, dbtable, jsonStatus, displayModel)
    {
        ...
        $("[issubflowtable='1']").each(function ()
        {
            ...
            else
            {
                ...
                var ajaxUrl = RoadUI.Core.rooturl() + "/Platform/WorkFlowRun/Scripts/Forms/GetSubTableData.aspx?secondtable=" + secondtable + "&primarytablefiled=" + primarytablefiled +
                        ...
                        for (var i = 0; i < json.length; i++)
                        {
                            ...
                            for (var j = 0; j < $tds.length; j++)
                            {
                                ...
                                $("[issubflow='1']", $tds.eq(j)).each(function ()
                                {
                                    ...
                                    var subtype = $ctl.attr("type1");
                                    var subvalidate = formrun.initFlowControl(subtype, $ctl, colvalue, "", subfieldstatus, displayModel);
        ...
        switch (type)
        {
            ...
            case "subflow_files":
                ...
                else if (filedshow == 1)
                {
                    ...
                    {
                        $.ajax({
                            url: RoadUI.Core.rooturl() + "/Platform/Files/GetShowString.ashx", data: { "files": initValue }, type: "post", success: function (links)
                            {
                                $control.next().remove();
                                $control.prev().remove();
                                $control.after(links);
    ...
    //pkValue：主键值 isAppend 0：为在当前行后增加 1：为在最后一行增加 isLoad:是否为二次加载 
    subtableNewRow: function (but, pkValue, isAppend, isLoad)
    {
        ...
        {
            var colname = $(this).attr("colname");
            $(this).attr("name", id + "_" + guid + "_" + colname).attr("id", id + "_" + guid + "_" + colname).attr("value1", $(this).attr("defaultvalue")).val("");
            $(this).prev().remove();
            $(this).next().remove();
