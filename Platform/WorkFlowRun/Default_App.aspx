<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Default_App.aspx.cs" Inherits="WebForm.Platform.WorkFlowRun.Default_App" %>
...
<form id="mainform" name="mainform" method="post" target="submiter">
    ...
    <div style="width:98%; margin: 0 auto;">
        ...
        <div>
        <select class="myselect" id="mycomment" style="margin-right:4px; width:80px;" onchange="$('#comment').val(this.value);"><option value=""></option><%=commentsOptions %></select><input type="text" class="mytext" id="comment" name="comment" value="" style="width:50%; margin-right:1px;" />
        <%if (signType == 2){%>
        <input type="hidden" value="" id="issign" name="issign" /><input type="button" class="mybutton" id="signbutton" onclick="sign();" value="&nbsp;&nbsp;签章&nbsp;&nbsp;"/>
        <%
            string signFile = string.Concat(Server.MapPath("../../Files/UserSigns/"), currentUserID, ".gif");
            string signSrc = string.Concat("../../Files/UserSigns/", currentUserID, ".gif");
            if (!System.IO.File.Exists(signFile))
            {
                System.Drawing.Bitmap img = new RoadFlow.Platform.WorkFlow().CreateSignImage(CurrentUserName);
                if (img != null)
                {
