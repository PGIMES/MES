    ...
    <table cellpadding="0" cellspacing="1" border="0" width="100%">
        <tr>
            <td>
                ...
                流程：<select class="myselect" multiple="multiple" style="width:100px;" id="FlowID" name="FlowID"><option value=""></option><asp:Literal ID="FlowOptions" runat="server"></asp:Literal></select>
                ...
                状态：<select class="myselect"  multiple="true" style="width:80px;" id="Status" name="Status" runat="server"></select>
...
<table class="listtable">
    ...
    %>
        <tr>
            <td><a href="javascript:void(0);" onclick="openTask('/Platform/WorkFlowRun/Default.aspx?<%=query1 %>','<%=task.Title %>','<%=task.ID %>');return false;" class="blue"><%=task.Title %></a></td>
            <td><%=flowName %></td>
            <td><%=stepName %></td>
...
<script type="text/javascript">
    ...
    {
        top.mainDialog.open({
            url: '/Platform/WorkFlowTasks/InstanceManage.aspx?flowid1=' + flowid + "&groupid=" + groupid + '<%=query%>',
            width: 1000, height: 450, title: "管理流程实例", opener:parent
        });
    }
    function delete1(flowid, groupid)
    {
        ...
        {
            $.ajax({
                url: "Delete.ashx?flowid1=" + flowid + "&groupid=" + groupid, async: false, cache: false, success: function (txt)
                {
                    alert(txt);
