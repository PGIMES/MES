...
<table cellpadding="0" cellspacing="1" border="0" class="listtable" style="width:100%; margin-top:0px;">
    ...
    <tbody>
    <% 
      foreach (var task1 in tasks.GroupBy(p => p.GroupID)){
           var tasksList = tasks.Where(p => p.GroupID == task1.Key);      
     %>
        <tr>
...
<%}else{
    ...
    foreach (var task in tasks.Where(p => p.GroupID == tasks.FirstOrDefault().GroupID))
    {
        tasksjson.Append("{");
