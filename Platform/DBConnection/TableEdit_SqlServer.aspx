<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TableEdit_SqlServer.aspx.cs" Inherits="WebForm.Platform.DBConnection.TableEdit_SqlServer" %>
...
<body>
            ...
            <tbody>
                <%
                    ...
                    foreach(System.Data.DataRow dr in SchemaDt.Rows)
                    {
                        ...
                        string type = dr["t_name"].ToString().ToLower();
                        if ("decimal" == type)
                        {
                            length = dr["length"].ToString() + "," + dr["SCALE"].ToString();
                        }
                        else if ("datetime" == type || "text" == type || "uniqueidentifier" == type || "int" == type || "money" == type || "float" == type)
                        {
                            length = "";
                        }
                        if (!hasIdentity && "1" == dr["isidentity"].ToString())
                        {
                            hasIdentity = true;
                        }
                %>
                <tr>
                    ...
                    <td><select class="myselect" name="<%=dr["f_name"] %>_type" onchange="typeChange(this)" style="width:90%"><%=DBConn.GetFieldDataTypeOptions(dr["t_name"].ToString(), "sqlserver") %></select></td>
    ...
    <script type="text/javascript">
        ...
        $(function ()
        {
            if ("True" == hasIdentity)
            {
                $("input[name$='_isidentity']").prop("disabled", true);
            }
        });
        function check()
        {
        ...
        function addRow()
        {
            ...
            $("input[name$='_isnull']", $tr1).attr('name', fname + '_isnull').val('1').prop('checked', true).prop('disabled', false);
            $("input[name$='_isidentity']", $tr1).attr('name', fname + '_isidentity').val('1').prop('checked', false);
            $("input[name$='_primarykey']", $tr1).attr('name', fname + '_primarykey').val('1').prop('checked', false);
