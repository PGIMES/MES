UE.compule = {
    ...
    getTextHtml: function ($control)
    {
        ...
        $control.removeAttr("defaultvalue").removeAttr("ondblclick").removeAttr("width1").removeAttr("format");
    },
    ...
    getSelectHtml: function ($control)
    {
        ...
        if ("0" == datasource)//数据字典
        {
            ...
            radios += '<%=BDictionary.GetOptionsByID("' + dictid + '".ToGuid(), ' + valuefiled + ', "' + defaultvalue + '", ' + ("1" == ischild ? 'true' : 'false') + ')%>';
            radios += '</select>';
            var $radios = $(radios);
    ...
    getGridHtml: function ($control)
    {
        ...
        div += '$(function(){$.ajax({url:"../WorkFlowFormDesigner/GetFormGridHtml.ashx<%=Request.Url.Query%>",data:{"dbconnid":"<%=DBConnID%>","dataformat":"' + $control.attr("dataformat") + '","datasource":"' + $control.attr("datasource") + '","datasource1":"' + $control.attr("datasource1") + '","params":params},type:"POST",success:function(html){$("#' + tableid + '").html(html);}});});';
        div += '';
        div += '</script>'
    ...
    getSubTableHtml_Text: function (colnumJSON, id, i, iscount)
    {
        ...
        input += 'defaultvalue="' + UE.compule.getDefaultValue(editmode.text_defaultvalue) + '" ';
        if (iscount)
        {
    ...
    getSubTableHtml_Textarea: function (colnumJSON, id, i, iscount)
    {
        ...
        textarea += 'defaultvalue="' + UE.compule.getDefaultValue(editmode.textarea_defaultvalue) + '"';
        if (scripts && $.isArray(scripts))
        {
            for (var i = 0; i < scripts.length; i++)
            {
    ...
    getSubTableHtml_Select: function (colnumJSON, id, i, iscount)
    {
        ...
        switch (ds)
        {
            case "select_dsdict":
                ...
                select += '<%=BDictionary.GetOptionsByID("' + rootid + '".ToGuid(), RoadFlow.Platform.Dictionary.OptionValueField.ID, "' + dvalue + '", ' + (select_ds_dict_ischild ? 'true' : 'false') + ')%>';
                break;
            case "select_dssql":
                ...
                select += '<%=new RoadFlow.Platform.WorkFlowForm().GetOptionsFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + dvalue + '")%>';
                break;
    ...
    getSubTableHtml_Checkbox: function (colnumJSON, id, i, iscount)
    {
        ...
        switch (ds)
        {
            ...
            case "checkbox_dssql":
                ...
                checkbox = '<%=new RoadFlow.Platform.WorkFlowForm().GetCheckboxFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + name + '", "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_checkbox\' colname=\'' + colnumJSON.name + '\'")%>';
                break;
    ...
    getSubTableHtml_Radio: function (colnumJSON, id, i, iscount)
    {
        ...
        switch (ds)
        {
            ...
            case "radio_dssql":
                ...
                radio = '<%=new RoadFlow.Platform.WorkFlowForm().GetRadioFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + name + '", "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_radio\' colname=\'' + colnumJSON.name + '\'")%>';
                break;
            case "radio_dsstring":
    ...
    getSubTableHtml_DateTime: function (colnumJSON, id, i, iscount)
    {
        var editmode = colnumJSON.editmode;
        var scripts = editmode.scripts;
        var datetime = '<input type="text" class="mycalendar" name="' + id + '_' + i + '_' + colnumJSON.name + '" id="' + id + '_' + i + '_' + colnumJSON.name + '" issubflow="1" type1="subflow_datetime" defaultvalue="' + UE.compule.getDefaultValue(editmode.datetime_defaultvalue) + '" value="' + UE.compule.getDefaultValue(editmode.datetime_defaultvalue) + '" ';
    ...
    {
        var editmode = colnumJSON.editmode;
        var org = '<input type="text" class="mymember" name="' + id + '_' + i + '_' + colnumJSON.name + '" id="' + id + '_' + i + '_' + colnumJSON.name + '" issubflow="1" type1="subflow_org" defaultvalue="' + UE.compule.getDefaultValue(editmode.org_defaultvalue) + '" value="' + UE.compule.getDefaultValue(editmode.org_defaultvalue) + '" ';
