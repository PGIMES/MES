...
; RoadUI.SelectDiv = function ()
{
    ...
    this.init = function ($divs)
    {
        $divs.each(function (index)
        {
            ...
            {
                $.ajax({
                    url: RoadUI.Core.rooturl() + "/Controls/SelectDiv/GetTitles.ashx?values=" + value + "&applibaryid=" + appid + "&titlefield=" + titlefield + "&pkfield=" + pkfield, type: "get", async: false, cache: false, success: function (txt)
                    {
                        $_div.val(txt);
                    }
                });
            ...
            else
            {
                $but.bind("click", function ()
                {
                    ...
                    new RoadUI.Window().open({ id: "selectdiv_" + id, url: "/Controls/SelectDiv/Default.aspx?" + params, width: width, height: height, resize: false, title: title, openerid: RoadUI.Core.query("tabid") || "" });
                });
            }
