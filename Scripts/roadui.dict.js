        ...
        $members.each(function (index)
        {
            ...
            if (value && value.length > 0)
            {
                switch (datasource)
                {
                    ...
                    default:
                        $.ajax({
                            url: RoadUI.Core.rooturl() + "/Controls/SelectDictionary/GetNames.ashx?values=" + encodeURI(value), type: "get", async: false, cache: false, success: function (txt)
                            {
                                $_member.val(txt);
                            }
                        });
                    ...
                    case "1":
                        ...
                        $.ajax({
                            url: RoadUI.Core.rooturl() + "/Controls/SelectDictionary/GetNames_SQL.ashx?values=" + encodeURI(value) + "&dbconn=" + dbconn + "&sql=" + sql, type: "get", async: false, cache: false, success: function (txt)
                            {
                                $_member.val(txt);
                            }
                        });
                    ...
                    case "2": //url
                        ...
                        $.ajax({
                            url: url, type: "get", async: false, cache: false, success: function (txt)
                            {
                                $_member.val(txt);
                            }
                        });
                    ...
                    case "3": //table
                        ...
                        url = RoadUI.Core.rooturl() + "/Controls/SelectDictionary/GetNames_Table.ashx?values=" + encodeURI(value);
            ...
            else
            {
                $but.bind("click", function ()
                {
                    ...
                    new RoadUI.Window().open({
                        id: "dict_" + id, url: "/Controls/SelectDictionary/" + (ismobile ? "Default_App.aspx" : "Default.aspx") + "?" + params, width: ismobile ? 320 : 500, height: ismobile ? 350 : 470, resize: false,
    ...
    this.setValue = function (objorid)
    {
        ...
        if (value && value.length > 0)
        {
            switch (datasource)
            {
                ...
                default:
                    $.ajax({
                        url: RoadUI.Core.rooturl() + "/Controls/SelectDictionary/GetNames.ashx?values=" + value, type: "get", async: false, cache: false, success: function (txt)
                        {
                            $obj.next().val(txt);
                        }
                    });
