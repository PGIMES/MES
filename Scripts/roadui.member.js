...
; RoadUI.Member = function ()
{
    ...
    this.init = function ($members)
    {
        $members.each(function (index)
        {
            ...
            {
                $.ajax({
                    url: RoadUI.Core.rooturl() + "/Controls/SelectMember/GetNames.ashx?values=" + value, type: "get", async: false, cache: false, success: function (txt)
                    {
                        $_member.val(txt);
                    }
                });
            ...
            else
            {
                $but.bind("click", function ()
                {
                    ...
                    var url = ismobile ? "/Controls/SelectMember/Default_App.aspx?" + params : "/Controls/SelectMember/Default.aspx?" + params;
                    new RoadUI.Window().open({ id: "member_" + id, url: url, width: ismobile ? 320 : 500, height: ismobile ? 350 : 470, resize: false, title: "选择组织机构成员", openerid: RoadUI.Core.query("tabid") || "" });
                });
            }
