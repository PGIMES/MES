...
public partial class Daoju_Daoju : System.Web.UI.Page
{
    ...
    private DataTable GvToDt()
    {
        ...
        if (ldt.Rows.Count > 0)
        {
            lsdaoju_no = ldt.Rows[0]["daoju_no"].ToString();
            lsdaoju_desc = ldt.Rows[0]["daoju_desc"].ToString();
            lslength = ldt.Rows[0]["length"].ToString();
        }
        for (int i = 1; i < ldt.Rows.Count; i++)
        {
            if (ldt.Rows[i]["daoju_no"].ToString() == lsdaoju_no || ldt.Rows[i]["daoju_desc"].ToString() == lsdaoju_desc || ldt.Rows[i]["daoju_no"].ToString().Trim() == "")
            {
            ...
            {
                lsdaoju_no = ldt.Rows[i]["daoju_no"].ToString();
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (this.m_sid == "")
        {
            ...
            //没有单号，自动生成
            string lsno = Pgi.Auto.Public.GetNo("DJ", "DJ");
            for (int i = 0; i < ls.Count; i++)
            {
        ...
        {
            //判断组号是否为空，如果为空新增
            if (lspgi_no != "" && lsop != "" && ldt.Rows[i]["daoju_no"].ToString().Trim() != "")
            {
