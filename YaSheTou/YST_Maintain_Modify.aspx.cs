...
public partial class YaSheTou_YST_Maintain_Modify : System.Web.UI.Page
{
    ...
    [WebMethod]
    public static string check_data(string part)
    {
        string re_flag = "";

        DataTable dt = DbHelperSQL.Query("select * from [qad].[dbo].[qad_pt_mstr] where pt_part like 'Z%' and pt_part='" + part + "'").Tables[0];
        if (dt == null)
        {
            re_flag = "物料号" + part + "不存在，不能新增！";
        }
        else
        {
            if (dt.Rows.Count <= 0)
